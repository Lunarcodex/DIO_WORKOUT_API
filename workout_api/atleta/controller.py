...
async def post(
    ...
    if not categoria:
        raise HTTPException(
        status_code=status.HTTP_400_BAD_REQUEST,
        detail=f'Categoria {categoria_name} não foi encontrada'
        )
...
@router.get(
        '/{id}', summary='Consultar um atleta pelo ID',
        status_code=status.HTTP_200_OK,
        response_model=AtletaOut,
)
...
@router.patch(
        '/{id}', summary='Editar um atleta pelo ID',
        status_code=status.HTTP_200_OK,
        response_model=AtletaOut,
)
async def update(id: uuid4, db_session: DataBaseDependency, atleta_up: AtletaIn=Body(...)
    ) -> AtletaOut:
    atleta: AtletaOut = (await db_session.execute(select(AtletaModel).filter_by(id=id))).scalars().first()
    if not atleta:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Atleta não encontrado no id {id}')
...
@router.delete(
        '/{id}', summary='Deletar um atleta pelo ID',
